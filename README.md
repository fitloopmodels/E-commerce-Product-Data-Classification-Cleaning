# 🛒 E-commerce Product Data Classifier
 
> A production-grade Python ETL pipeline that ingests raw, messy e-commerce product records — cleans them, classifies them into a structured taxonomy, extracts key attributes, and scores each record for data quality.
 
![Python](https://img.shields.io/badge/Python-3.9%2B-blue?style=flat-square&logo=python)
![License](https://img.shields.io/badge/License-MIT-green?style=flat-square)
![Tests](https://img.shields.io/badge/Tests-41%20passing-brightgreen?style=flat-square)
![No ML](https://img.shields.io/badge/ML%20deps-none-orange?style=flat-square)
...
**Results on 1,200 records:**
- ✅ 98%+ classification rate
- ✅ Removes ~8% exact/near-duplicates automatically
- ✅ Nullifies 1,600+ junk placeholder values (`"N/A"`, `"TBD"`, `"???"`, etc.)
...
# 2. Run the full pipeline (auto-generates 1,200 messy records)
python main.py
...
**Outputs written to:**
- `data/processed/products_cleaned.csv` — enriched 28-column dataset
- `data/processed/products_poor_quality.csv` — records flagged for manual review
- `reports/quality_report.json` — full QA summary
---
...
|------|---------|-------------|
| `--input` | _(auto-generate)_ | Path to your own CSV |
| `--n` | `1200` | Number of synthetic records to generate |
| `--no-generate` | `False` | Fail if raw CSV is missing instead of generating |
| `--taxonomy` | `data/taxonomy/taxonomy.json` | Path to a custom taxonomy file |
 
```bash
python main.py --n 5000
python main.py --no-generate
python main.py --taxonomy path/to/custom_taxonomy.json
```
...
|------|--------|--------------|
| 1 | `data_generator.py` | Generates 1,200+ realistic, intentionally messy product records |
...
4. Assigns a confidence level based on hit count
...
| `low` | 1 keyword hit |
| `none` | 0 hits → fallback applied |
 
> Every classification decision is logged with its keyword score — fully auditable, no black box.
 
---
 
## 🗃️ Taxonomy
 
The taxonomy lives in `data/taxonomy/taxonomy.json` and maps:
...
> To add a new category, edit `taxonomy.json` — no code changes needed.
 
---
 
## 📥 Input Schema
 
Your CSV must include these columns (others are ignored):
...
| `brand`, `category`, `subcategory` | Cleaner | Standardized |
| `price`, `stock_quantity`, `rating` | Cleaner | Coerced to correct types |
...
| `extracted_brand` | Extractor | Brand parsed from name |
...
- Price, rating, and stock type coercion
...
| `re` (stdlib) | Regex-based attribute extraction |
...
Replace `classifier.py` with a fine-tuned text classifier using `sentence-transformers` + cosine similarity against category descriptions.
 
**Add a REST API**
Wrap `pipeline.py` in FastAPI to expose `/classify` and `/clean` endpoints.
 
**Add a database sink**
Replace the CSV output in `pipeline.py` with a SQLAlchemy write to PostgreSQL.
 
**Connect real data**
Point `--input` at an exported CSV from Shopify, WooCommerce, or any PIM system.
...
<p align="center">Built with Python · No ML required · Fully auditable</p>
